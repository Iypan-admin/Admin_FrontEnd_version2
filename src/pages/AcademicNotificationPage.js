...
const AcademicNotificationPage = () => {
  ...
  const getNotificationType = (notif) => {
    ...
    if (type === 'STUDENT_REGISTERED' || (type.includes('STUDENT') && message.includes('registered'))) {
      return { label: 'Student Registration', color: 'bg-purple-100 text-purple-800' };
    }
    ...
    if (type.includes('STUDENT') || message.includes('student')) {
      return { label: 'Student', color: 'bg-purple-100 text-purple-800' };
    }
    if (type.includes('TEACHER') || message.includes('teacher')) {
      return { label: 'Teacher', color: 'bg-orange-100 text-orange-800' };
    }
...
export default AcademicNotificationPage;
