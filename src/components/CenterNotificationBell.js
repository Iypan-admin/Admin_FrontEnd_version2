...
import { getCenterNotifications, markCenterNotificationAsRead } from '../services/Api';

const CenterNotificationBell = () => {
    ...
    const getNotificationAvatar = (notif) => {
        ...
        if (type.includes('batch') || message.includes('batch')) {
            return { bg: '#2196f3', icon: '📦', text: 'B' };
        }
        if (type.includes('student') || message.includes('student')) {
            return { bg: '#10b981', icon: '🎓', text: 'S' };
        }
        if (type.includes('teacher') || message.includes('teacher')) {
            return { bg: '#3b82f6', icon: '👤', text: 'T' };
        }
    ...
    useEffect(() => {
        ...
        const fetchNotifications = async (isInitialLoad = false) => {
            ...
            try {
                const res = await getCenterNotifications(token);
                if (res && res.success) {
    ...
    const handleMarkAsRead = async (notificationId, e) => {
        ...
        try {
            await markCenterNotificationAsRead(notificationId, token);
            setNotifications((prev) => prev.filter((n) => n.id !== notificationId));
        } catch (err) {
            console.error("Failed to mark center notification as read:", err);
        }
    };
    ...
    return (
        <div className="relative">
            <button
                ...
                className="relative p-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors group"
                title="View Notifications"
            >
                <svg className="w-5 h-5 sm:w-6 sm:h-6 group-hover:text-blue-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {showDropdown && (
                <>
                    ...
                    ></div>
                    <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-lg shadow-xl border border-gray-200 z-50 overflow-hidden transform origin-top-right transition-all">
                        {/* Header */}
...
export default CenterNotificationBell;
