...
import { getFinanceNotifications, markFinanceNotificationAsRead } from '../services/Api';
...
const FinanceNotificationBell = () => {
  ...
  const getNotificationAvatar = (notif) => {
    ...
    if (type.includes('invoice') || message.includes('invoice')) {
      return { bg: '#3b82f6', icon: '📄', text: 'I' };
    }
    if (type.includes('refund') || message.includes('refund')) {
      return { bg: '#ef4444', icon: '↩️', text: 'R' };
    }
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async (isInitialLoad = false) => {
      ...
      try {
        const res = await getFinanceNotifications(token);
        if (res && res.success) {
  ...
  const handleMarkAsRead = async (notificationId, e) => {
    ...
    try {
      await markFinanceNotificationAsRead(notificationId, token);
      setNotifications((prev) => prev.filter((n) => n.id !== notificationId));
    } catch (err) {
      console.error("Failed to mark finance notification as read:", err);
    }
  };
          ...
          <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-lg shadow-xl border border-gray-200 z-50 overflow-hidden">
            ...
            <div className="px-4 py-3 border-b border-gray-200 bg-white">
              <div className="flex items-center justify-between">
                ...
                {unreadCount > 0 && (
                  <span className="text-xs font-medium px-3 py-1 rounded-full text-white" style={{ backgroundColor: '#2196f3' }}>
                    {unreadCount} New
                  </span>
                )}
