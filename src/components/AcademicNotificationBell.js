...
import { getAcademicNotifications, markAcademicNotificationAsRead } from '../services/Api';

const AcademicNotificationBell = () => {
  ...
  const getNotificationAvatar = (notif) => {
    ...
    if (type.includes('batch_assigned') || type.includes('batch_created')) {
      return {
        bg: '#3b82f6', // Blue
        icon: '📚',
        text: 'B'
    ...
    if (type.includes('batch_rejected')) {
      return {
        bg: '#ef4444', // Red
        icon: '✕',
        text: '✕'
    ...
    if (type.includes('student_registered') || type === 'STUDENT_REGISTERED') {
      return {
        bg: '#8b5cf6', // Purple
        icon: '👤',
        text: 'S'
    ...
    if (type.includes('student_approved')) {
      return {
        bg: '#10b981', // Green
        icon: '✓',
        text: '✓'
    ...
    if (type.includes('teacher_assigned')) {
      return {
    ...
    if (type.includes('teacher_leave_request') || type === 'TEACHER_LEAVE_REQUEST') {
      return {
        bg: '#f59e0b', // Orange/Amber
        icon: '📝',
        text: 'L'
    ...
    if (type.includes('batch_request') || type === 'BATCH_REQUEST') {
      return {
        bg: '#3b82f6', // Blue
        icon: '📋',
        text: 'R'
    ...
    if (message.includes('student') && (message.includes('registered') || message.includes('approved'))) {
      return {
        bg: '#8b5cf6', // Purple
        icon: '👤',
        text: 'S'
    ...
    if (message.includes('batch') && (message.includes('created') || message.includes('assigned'))) {
      return {
        bg: '#3b82f6', // Blue
        icon: '📚',
        text: 'B'
    ...
    if ((message.includes('center') && message.includes('submitted')) || message.includes('batch request')) {
      return {
        bg: '#3b82f6', // Blue
        icon: '📋',
        text: 'R'
    ...
    if (message.includes('leave') && message.includes('requesting')) {
      return {
        bg: '#f59e0b', // Orange/Amber
        icon: '📝',
        text: 'L'
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async (isInitialLoad = false) => {
      ...
      try {
        const res = await getAcademicNotifications(token);
        if (res.success) {
          // Filter out read notifications - only show unread ones in the dropdown
  ...
  const handleMarkAsRead = async (notificationId, e) => {
    ...
    try {
      await markAcademicNotificationAsRead(notificationId, token);
                ...
                {notifications.length > 0 && (
                  <button 
                    onClick={() => {
                      ...
                      navigate('/academic/notifications');
                    }}
...
export default AcademicNotificationBell;
