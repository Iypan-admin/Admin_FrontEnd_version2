...
import { getResourceNotifications, markResourceNotificationAsRead } from '../services/Api';

const ResourceNotificationBell = () => {
  ...
  const getNotificationAvatar = (notif) => {
    ...
    // Resource related avatars
    if (type.includes('resource') || message.includes('resource')) {
      return {
        bg: '#3b82f6', // Blue
        icon: '📂',
        text: 'R'
    ...
    if (type.includes('upload') || message.includes('upload')) {
        return {
          bg: '#10b981', // Green
          icon: '📤',
          text: 'U'
    ...
    if (type.includes('lsrw') || message.includes('lsrw')) {
      return {
        bg: '#8b5cf6', // Purple
        icon: '📚',
        text: 'L'
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async (isInitialLoad = false) => {
      ...
      try {
        const res = await getResourceNotifications(token);
        if (res.success) {
  ...
  const handleMarkAsRead = async (notificationId, e) => {
    ...
    try {
      await markResourceNotificationAsRead(notificationId, token);
                ...
                {notifications.length > 0 && (
                  <button 
                    onClick={() => {
                      ...
                      navigate('/resource-manager/notifications');
                    }}
              ...
              {loading ? (
                ...
                <div>
                  {notifications.map((notif) => {
                    ...
                    const hasTitle = messageParts.length > 2; // Usually title \n message
...
export default ResourceNotificationBell;
